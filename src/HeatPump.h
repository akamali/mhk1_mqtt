...
#include <functional>
#define SETTINGS_CHANGED_CALLBACK_SIGNATURE std::function<void(heatpumpSettings const& newSettings)> settingsChangedCallback
#define STATUS_CHANGED_CALLBACK_SIGNATURE std::function<void(heatpumpStatus const& newStatus)> statusChangedCallback
...
#define STATUS_CHANGED_CALLBACK_SIGNATURE void (*statusChangedCallback)(heatpumpStatus const& newStatus)
...
class heatpumpFunctions  {
  private:
    ...
    int getCode(byte b) const;
    int getValue(byte b) const;
  ...
  public:
    ...
    void setData1(byte const* data);
    void setData2(byte const* data);
    ...
    heatpumpFunctionCodes getAllCodes() const;
...
class HeatPump
{
  ...
  public:
    ...
    int readPacket(byte const* packet, int const length);


    HeatPump();
    void update(byte* packet);
    ...
    void clearFunctions();
    bool setFunctions1(byte* packet1);
    bool setFunctions2(byte* packet2);
    ...
    static byte checkSum(byte const bytes[], int len);
};
#endif
